...
const maleFirstNames = [
    'Александр', 'Дмитрий', 'Максим', 'Сергей', 'Андрей', 'Алексей', 'Артем', 'Иван',
    'Михаил', 'Евгений', 'Владимир', 'Николай', 'Виктор', 'Павел', 'Роман', 'Олег',
    'Василий', 'Георгий', 'Константин', 'Тимофей'
];

// Женские имена
const femaleFirstNames = [
    'Анна', 'Мария', 'Елена', 'Ольга', 'Татьяна', 'Наталья', 'Ирина', 'Светлана', 'Юлия',
    'Екатерина', 'Анастасия', 'Виктория', 'Ксения', 'Полина', 'Дарья', 'Алина', 'Вероника',
    'Валерия', 'Александра', 'Евгения'
];

// Мужские фамилии (оканчиваются на согласную/«ов», «ев» и т. д.)
const maleLastNames = [
...
// Женские фамилии (оканчиваются на «ова», «ева», «ина» и т. д.)
const femaleLastNames = [
    'Григорьева', 'Александрова', 'Кузнецова', 'Попова', 'Соколова', 'Иванова',
    'Петрова', 'Сидорова', 'Смирнова', 'Васильева',
    'Андреева', 'Макарова', 'Никитина', 'Захарова', 'Зайцева', 'Соловьева', 'Борисова', 'Яковлева'
];

// Остальные массивы (без изменений)
const departments = [
...
// Генерация реалистичных данных с учётом соотношения фамилий
const generateTestData = () => {
    const employees = [];
    const usedFullNames = new Set();

    // Рассчитываем соотношение: вероятность женского пола = кол-во женских фамилий / общее кол-во фамилий
    const totalLastNames = maleLastNames.length + femaleLastNames.length;
    const femaleProbability = femaleLastNames.length / totalLastNames;

    for (let i = 1; i <= 20000; i++) {
        // Определяем пол с вероятностью, соответствующей соотношению фамилий
        const isFemale = Math.random() < femaleProbability;
        ...
        if (usedFullNames.has(fullName)) {
            continue; // Пропускаем итерацию, если имя уже есть
        }
        usedFullNames.add(fullName);

        // Генерируем email
        const email = `${firstName.toLowerCase()}.${lastName.toLowerCase()}@company.ru`;
